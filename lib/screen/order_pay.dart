import 'package:app_thuong_mai/Item/order_pay_item.dart';
import 'package:app_thuong_mai/Item/ordername.dart';
import 'package:app_thuong_mai/screen/loading_pay.dart';
import 'package:app_thuong_mai/screen/notifications.dart';
import 'package:firebase_database/firebase_database.dart';
import 'package:flutter/material.dart';

class OrderPay extends StatefulWidget {
  final int userToken;
  const OrderPay({super.key, required this.userToken});
...
class _OrderPayState extends State<OrderPay> {
  ...
  Future<void> addNotication() async {
    try{
      await _databaseReference.child('users/${widget.userToken}').child('notifications/${notificationNumber}').set({
        'status': true,
        'title': 'Bạn vừa đặt thành công 1 đơn hàng'
      });
        notificationNumber++;
      await _databaseReference.child('users/${widget.userToken}').update({
  ...
  Future<void> setCategory() async {
    try{
      for(int i = 0 ; i<lst_cat.length;i++){
        await _databaseReference.child('users/${widget.userToken}').child('cats/${lst_cat[i]['cat_token']}').update({
          'status': false,
        });
      }
    }catch(e){
  ...
  Future<void> addNewOrder() async {
    try {
      for(int i = 0; i<lst_order.length;i++){
        await _databaseReference.child('users/${widget.userToken}').child('orders/${orderNumber}').child('${i}').set({
          ...
          actions: <Widget>[
            ...
            TextButton(
              onPressed: () {
                ...
                Navigator.push(context, MaterialPageRoute(builder: (context)=>SplashScreen(userToken: widget.userToken,)));
